...
## Usage
Generate the warpped label first using  **labelwarp.m** and run **SetupData.m** to get the predicted label map from the trained model, and the dice score is computed to show the quality of the segmentation. Code works on both 2D and 3D T1 MR images where some hyperparameters tuning need to be made accordingly.
